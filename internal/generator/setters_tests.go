...
func parseSetterType(setter tgSetter, counter *int) string {
	...
	case "*KeyboardButtonRequestManagedBot":
		*counter++
		return fmt.Sprintf("&KeyboardButtonRequestManagedBot{RequestID: %d}", *counter)
	case "[]InputSticker":
		return "[]InputSticker{{}}"
	case "InputSticker":
	...
	case "InputChecklist":
		return "InputChecklist{Tasks: []InputChecklistTask{{}}}"
